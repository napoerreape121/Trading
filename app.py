...
if st.button("🚀 Ejecutar Escáner General y Despachar Gestión"):
    # PARTE 1: ANALIZADOR DE GESTIÓN ACTIVA PARA TUS COMPRAS REALES EN EL DISCO DURO
    if not df_portafolio.empty:
        ...
        try:
            ...
            for idx, row in df_portafolio.iterrows():
                ...
                ema50_v = df_t_cart['Close'].ewm(span=50, adjust=False).mean().iloc[-1]
                ...
                # REGLA 1: VALIDAR SALIDA INMEDIATA POR MATRIZ DE PRECIOS
                if low_vivo <= float(row["StopLoss"]):
                    msg_sl = f"🚨 *¡ALERTA CRÍTICA DE SALIDA!* 🚨\n\n📉 El CEDEAR `{tick.split('.')[0]}` perforó tu *Stop Loss* de ${row['StopLoss']:,.2f}.\n🛒 *Acción:* VENDER de inmediato tus {int(row['Cantidad'])} unidades en Balanz para frenar pérdidas."
                    enviar_alerta_telegram(msg_sl)
                    st.error(f"¡Alerta de Venta Enviada para {tick} por Stop Loss Perforado!")
                    continue
                elif high_vivo >= float(row["TakeProfit"]):
                    msg_tp = f"🎯 *¡ALERTA DE OBJETIVO CUMPLIDO!* 🎯\n\n📈 El CEDEAR `{tick.split('.')[0]}` tocó tu *Take Profit* de ${row['TakeProfit']:,.2f}.\n🛒 *Acción:* VENDER tus {int(row['Cantidad'])} unidades en Balanz y retirar ganancias netas."
