...
# =====================================================================
st.set_page_config(
    page_title="Simulador Cuantitativo CEDEARs", 
    page_icon="📈", 
    layout="wide"
)

st.title("📈 Simulador Cuantitativo Estricto de CEDEARs")
st.markdown("Estrategia de Confluencias (EMA 200, EMA 9, MACD, RSI 40-65, Volumen) con Control de Capital y Gestión de Riesgo Real.")
...
# Costos operativos (Comisión estimada + Derechos + IVA)
...
# =====================================================================
# 🚀 MOTOR DE BACKTESTING SINCRONIZADO
# =====================================================================
if st.button(f"🧪 Ejecutar Simulador ({opcion_tiempo})"):
    with st.spinner(f"Descargando datos históricos y calculando indicadores..."):
        try:
    ...
    if datos_globales is not None and not datos_globales.empty:
        ...
        if fechas_unicas and st.session_state.diccionario_precios_historicos:
            ...
            for f in range(total_fechas - 1):
                ...
                # FASE 1: Gestionar salidas (Stop Loss y Take Profit) en las ruedas siguientes
                for t_activo, pos in list(posiciones_activas.items()):
                    ...
                    if fecha_manana in df_activo.index:
                        ...
                        if cerrar:
                            ...
                            # Trailing stop dinámico opcional con EMA 9 y ATR
                ...
                # FASE 2: Buscar señales al cierre de HOY y comprar al cierre exacto de HOY (o apertura de MAÑANA)
                ...
                for tick, df_activo in st.session_state.diccionario_precios_historicos.items():
                    ...
                    if fecha_hoy in df_activo.index:
                        ...
                        # Filtros estrictos de confluencia
                        ...
                        if cond_tendencia and cond_momentum and cond_gatillo:
                            ...
                            target = p_neto_compra + ((p_neto_compra - stop_loss) * 2)
                            ...
                            posiciones_activas[tick] = {
                                ...
                                "TakeProfit": target,
                                "Cantidad": cantidad,
                                "FechaEntrada": fecha_hoy  # Sincronizado exactamente con el día de la señal y cierre
                            }
...
if "df_trades_global" in st.session_state and st.session_state.df_trades_global is not None:
    ...
    st.success("✅ ¡Backtest ejecutado y sincronizado con éxito!")
        ...
        if elegido:
            ...
            if tick_aud in st.session_state.diccionario_precios_historicos:
                ...
                if not df_rec.empty:
                    ...
                    # Anotaciones perfectamente alineadas a la fecha de compra exacta
    ...
    st.info("No se registraron operaciones que cumplieran con todas las confluencias estrictas en este período.")
