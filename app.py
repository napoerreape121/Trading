...
st.write(
    "Sistema Cuantitativo Estricto: El bot filtra oportunidades usando confluencias de Tendencia en Wall Street (EMA 200), "
    "Momentum en Dólares (MACD + RSI), Gatillos de Volumen Real en NY, y gestión de riesgo Anti-CCL."
)

# Uso de st.secrets con fallback a variables de entorno
...
if st.button("🚀 Ejecutar Escáner General y Despachar Gestión"):
    ...
    # ==========================================
    # PARTE 2: BUSCADOR CUANTITATIVO DE CONFLUENCIAS (ESTRICTO EN WALL STREET)
    # ==========================================
    with st.spinner("Ejecutando análisis técnico y validación de volumen en Wall Street (US)..."):
        try:
            tickers_us = [t.replace(".BA", "") for t in tickers_escaner]
            datos_mercado_us = descargar_mercado(tickers_us, period="2y")
            datos_mercado_local = descargar_mercado(tickers_escaner, period="5d")
        except Exception as e:
            st.error(f"Error en la descarga de mercado global: {e}")
            datos_mercado_us = None
            datos_mercado_local = None

    if datos_mercado_us is not None and not datos_mercado_us.empty and datos_mercado_local is not None:
        candidatos_validos = []
        tickers_en_cartera = set(df_portafolio_activo["Ticker"].tolist()) if not df_portafolio_activo.empty else set()
        riesgo_maximo_ars = capital_disponible * 0.02  # Regla inamovible del 2%

        for ticker_ba in tickers_escaner:
            if ticker_ba in tickers_en_cartera:
                continue
            
            ticker_us = ticker_ba.replace(".BA", "")
            
            try:
                # -------------------------------------------------------------
                # 1. ANÁLISIS TÉCNICO EXCLUSIVO EN WALL STREET (USD)
                # -------------------------------------------------------------
                df_us = extraer_ohlc(datos_mercado_us, ticker_us)

                df_us["EMA_9"] = ta.trend.ema_indicator(df_us["Close"], window=9)
                df_us["EMA_200"] = ta.trend.ema_indicator(df_us["Close"], window=200)
                df_us["RSI_14"] = ta.momentum.rsi(df_us["Close"], window=14)
                df_us["MACD_Hist"] = ta.trend.MACD(df_us["Close"]).macd_diff()
                df_us["ATR_14"] = ta.volatility.average_true_range(df_us["High"], df_us["Low"], df_us["Close"], window=14)
                df_us["Volumen_SMA_9"] = df_us["Volume"].rolling(window=9).mean()

                df_us = df_us.dropna()
                if len(df_us) < 2:
                    continue

                last_us = df_us.iloc[-1]
                prev_us = df_us.iloc[-2]

                Precio_Cierre_USD = last_us["Close"]
                Precio_Apertura_USD = last_us["Open"]
                Volumen_Actual_USD = last_us["Volume"]
                
                EMA_200_USD = last_us["EMA_200"]
                EMA_9_USD = last_us["EMA_9"]
                MACD_Hist_Actual = last_us["MACD_Hist"]
                MACD_Hist_Previo = prev_us["MACD_Hist"]
                RSI_Actual = last_us["RSI_14"]
                Volumen_SMA_9_USD = last_us["Volumen_SMA_9"]
                ATR_Actual_USD = last_us["ATR_14"]

                # -------------------------------------------------------------
                # 2. GATILLO BOOLEANO ESTRICTO (BASADO EN NY)
                # -------------------------------------------------------------
                Condicion_Tendencia = (Precio_Cierre_USD > EMA_200_USD)
                Condicion_Momentum = (Precio_Cierre_USD > EMA_9_USD) and (MACD_Hist_Actual > MACD_Hist_Previo) and (40 < RSI_Actual < 65)
                Condicion_Gatillo = (Precio_Cierre_USD > Precio_Apertura_USD) and (Volumen_Actual_USD > Volumen_SMA_9_USD)
                
                SEÑAL_COMPRA_US = Condicion_Tendencia and Condicion_Momentum and Condicion_Gatillo

                if not SEÑAL_COMPRA_US:
                    continue

                # -------------------------------------------------------------
                # 3. MAPEO LOCAL Y RIESGO ANTI-CCL
                # -------------------------------------------------------------
                df_local = extraer_ohlc(datos_mercado_local, ticker_ba)
                Precio_Cierre_Local = df_local["Close"].iloc[-1]
                
                Stop_Loss_USD = Precio_Cierre_USD - (2 * ATR_Actual_USD)
                Target_USD = Precio_Cierre_USD + ((Precio_Cierre_USD - Stop_Loss_USD) * 2)

                if Stop_Loss_USD <= 0:
                    continue

                factor_conversion_pesos = Precio_Cierre_Local / Precio_Cierre_USD
                
                Precio_Neto_Entrada_ARS = precio_compra_neto(Precio_Cierre_Local)
                Stop_Loss_ARS = Stop_Loss_USD * factor_conversion_pesos
                Target_ARS = Target_USD * factor_conversion_pesos

                Riesgo_por_Accion_ARS = Precio_Neto_Entrada_ARS - Stop_Loss_ARS

                if Stop_Loss_ARS <= 0 or Riesgo_por_Accion_ARS <= 0:
                    continue

                Cantidad = int(riesgo_maximo_ars // Riesgo_por_Accion_ARS)
                if Cantidad <= 0:
                    continue

                Costo_Total_ARS = Precio_Neto_Entrada_ARS * Cantidad
                if Costo_Total_ARS > capital_disponible:
                    continue

                candidatos_validos.append({
                    "Ticker": ticker_ba,
                    "PrecioNetoEntrada": round(Precio_Neto_Entrada_ARS, 2),
                    "StopLoss": round(Stop_Loss_ARS, 2),
                    "Target": round(Target_ARS, 2),
                    "Cantidad": Cantidad,
                    "CostoTotal": round(Costo_Total_ARS, 2)
                })
        ...
        if candidatos_validos:
            ...
            st.success("🎯 ¡Señal Validada en Wall Street! Estructura de riesgo aislada de la volatilidad del CCL.")
            ...
            msg_tg = (
                f"🟩 *SEÑAL DE COMPRA (WALL STREET CONFIRMADO)* 🟩\n\n"
                f"📌 *Ticker:* `{mejor_opcion['Ticker']}`\n"
                f"⚖️ *Nominales:* {int(mejor_opcion['Cantidad'])}\n"
                f"💲 *Entrada Neta:* `${mejor_opcion['PrecioNetoEntrada']:,.2f}`\n\n"
                f"⛔ *Stop Loss (Anti-CCL):* `${mejor_opcion['StopLoss']:,.2f}`\n"
                f"🎯 *Target (Ratio 1:2):* `${mejor_opcion['Target']:,.2f}`\n\n"
                f"✅ _Filtros NY OK: Tendencia USD + Momentum + Volumen Real USA._"
            )
            enviar_alerta_telegram(msg_tg)
        else:
            st.info("Ningún activo cumplió con las confluencias estrictas en Wall Street durante la sesión.")
