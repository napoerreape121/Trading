...
portafolio_activo = []  # Lista de posiciones abiertas en la simulación
historial_operaciones = []  # Registro de trades cerrados

# Aplanamos las fechas para recorrer día por día (Simulación secuencial temporal)
# Tomamos como referencia el índice de fechas de uno de los tickers
fechas = datos.index

print(f"🔄 Iniciando simulación temporal desde {fechas[MIN_VELAS].strftime('%Y-%m-%d')} hasta {fechas[-1].strftime('%Y-%m-%d')}...\n")

# Simulamos día por día (empezando desde el índice MIN_VELAS para tener historia para la EMA200)
for i in range(MIN_VELAS, len(fechas)):
    ...
    for ticker in tickers_prueba:
        ...
        try:
            # Cortar datos hasta la fecha actual de la simulación
            df_t = datos.loc[:fecha_actual, ticker].dropna()
            if len(df_t) < MIN_VELAS:
                continue
                
            # Calcular indicadores al día de la fecha
            ...
            # Reglas estrictas de confluencia
            ...
            # Cálculo de posición basada en el 2% del capital total actual
            ...
            # RESTRICCIÓN DE DINERO REAL: Si no hay efectivo suficiente, no se compra
